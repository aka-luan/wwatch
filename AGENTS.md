...
- Non-obvious gotcha: in local Node mode there is no `executionCtx.waitUntil`. `deferFrom` falls back to `void work`, so scans still run in the background. The frontend surfaces results through its 2.5s polling loop (`setInterval(refresh, 2500)` in `public/app.js`).
- To exercise a full scan end-to-end without a real WordPress site, point a site at a local fake WP server that answers `/wp-json`, `/wp-json/wp/v2/plugins`, and `/wp-json/wp-site-health/...` (see `scripts/verify.ts`, which stands one up and is run by `npm run verify`). Scans reach out to `api.wordpress.org` for latest core/plugin versions; that call is optional and failures are handled gracefully.
