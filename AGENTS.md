...
- To exercise a full scan end-to-end without a real WordPress site, point a site at a local fake WP server that answers `/wp-json`, `/wp-json/wp/v2/plugins`, and `/wp-json/wp-site-health/...` (see `scripts/verify.ts`, which stands one up and is run by `npm run verify`). Auto-login needs `POST /wp-json/wwatch/v1/login-link`. Updates need `GET /wp-json/wwatch/v1` and `POST /wp-json/wwatch/v1/update`. Scans reach out to `api.wordpress.org` for latest core/plugin versions; that call is optional and failures are handled gracefully.
